...
def make_and_save_qc_plots_xml_based(dataset_path, scratch_root, results_root="/results/tile_qc_plots"):
    """
    Generate and save quality control plots using XML-based camera alignment transforms.
    
    Creates comparative visualizations between original tiles and tiles with camera 
    alignment transforms applied from XML metadata, without requiring pre-computed 
    corrected tile data.
    ...
    dataset_path : str
        Path to the original/raw dataset directory containing zarr tiles
    scratch_root : str
        Path to scratch directory containing updated.M.txt file with transforms
    results_root : str, default="/results/tile_qc_plots"
    ...
    Notes
    -----
    - Reads transforms from updated.M.txt file generated by calc_affine()
    - Applies transforms on-the-fly to original tiles for comparison
    - Generates plots for each pair of channels that were aligned
    - Handles both single-digit channel identifiers and full wavelength strings
    - Excludes channel 405 from transform application as it serves as reference
    """
    import shutil
    ...
    # Load the affine transforms from scratch directory
    affine_file = Path(scratch_root) / 'updated.M.txt'
    if not affine_file.exists():
        print(f"Warning: Affine transform file not found at {affine_file}")
        return
    
    # Parse the transforms
    transforms = {}
    with open(affine_file) as f:
        import csv
        reader = csv.reader(f, delimiter='\t')
        for row in reader:
            if len(row) >= 7:  # channel + 6 affine parameters
                channel = row[0]
                transform_params = [float(x) for x in row[1:7]]
                transforms[channel] = transform_params
    ...
    for c1, c2 in pairs_of_channels:
        ...
        for i in range(tiles_to_process):
            ...
            tile_c2 = raw_tiles_c2[i]
            
            # Get Z planes to sample
            z_planes = get_z_planes(tile_c1, level=0, thickness=20, spacing=10)
            
            # Process subset of Z planes
            max_z_planes = 3  # Limit number of Z planes
            z_planes = z_planes[:max_z_planes]
            
            for z in z_planes:
                # Load raw data
                raw_c1 = load_raw_zarr_slice(tile_c1, z)
                raw_c2 = load_raw_zarr_slice(tile_c2, z)
                
                # Apply transforms to simulate corrected data
                corrected_c1 = apply_transform_to_slice(raw_c1, c1, transforms)
                corrected_c2 = apply_transform_to_slice(raw_c2, c2, transforms)
                
                # Generate filename
                tilename = Path(tile_c1).stem.split('_ch')[0]
                filename = f"{results_root}/{tilename}_z-{z}_{c1}-{c2}_channel_overlay"
                
                # Create overlay comparison plot
                overlay_images_rgb_zoom(
                    raw_c1, raw_c2, 
                    corrected_c1, corrected_c2, 
                    filename
                )
    
    # Copy affine transform file to results for reference
...
def apply_transform_to_slice(image_slice, channel, transforms):
    """
    Apply camera alignment transform to a single image slice.
    
    Parameters
    ----------
    image_slice : np.ndarray
        2D image slice to transform
    channel : str
        Channel identifier
    transforms : dict
        Dictionary mapping channels to transform parameters
        
    Returns
    -------
    np.ndarray
        Transformed image slice, or original if no transform available
    """
    if channel not in transforms or channel == '405':
        # No transform for this channel or it's the reference channel
        return image_slice
    
    # Get transform parameters [A, B, dX, D, E, dY]
    transform_params = transforms[channel]
    A, B, dX, D, E, dY = transform_params
    
    # Create 2D affine matrix
    ...
    # Apply transform using scipy
    # Convert to inverse transform for scipy (it uses inverse convention)
    inv_matrix = numpy.linalg.inv(transform_matrix[:2, :2])
    offset = -np.dot(inv_matrix, transform_matrix[:2, 2])
    ...
    return transformed.astype(image_slice.dtype)
